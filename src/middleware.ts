...
const AUTH_MAX = 30 // aumentado de 10 a 30
...
export function middleware(req: NextRequest) {
  ...
  if (pathname === '/api/auth/callback/credentials') {
    ...
    const { allowed, remaining, resetAt } = checkRateLimit(`login:${ip}`, AUTH_MAX)
    if (!allowed) {
      return NextResponse.json(
        { error: 'Demasiados intentos de login. Esperá 1 minuto.', retryAfter: Math.ceil((resetAt - Date.now()) / 1000) },
